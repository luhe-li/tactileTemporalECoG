...
fig_dir = fullfile(projectDir, 'derivatives','ECoGFigures',sprintf('sub-%s', subject),'frequency_analysis');
...
xlim([0, 200])
saveas(gca, fullfile(fig_dir, 'Frequency spectrum of CAR data'),'png')

xlim([0, 20])
saveas(gca, fullfile(fig_dir, 'Frequency spectrum of CAR data up 20hz'),'png')
...
figure;
plot(freqs, amp,'HandleVisibility','off');
yscale('log');
xlim([0 1])
ylim([1 10^7])
x = xline(f0,'r','DisplayName','Sweep frequency');
legend
xlabel('Frequency')
ylabel('Log amplitude')
saveas(gca, fullfile(fig_dir, 'Frequency spectrum of detrended data'),'png')
...
for ii = 1:show_n_ch
    ...
    top_channel_name{ii} = channels.name{idx_ch};
end

%% plot top coherence channel broadband power
...
specs.plot_ylim   = [-1, 4];
specs.stim_names  = fingers;

specs.chan_names  = top_channel_name;
