function bidsEcogBroadbandPlotAllchannels(projectDir, subject, sessions, tasks, runnums, ...
...
%% DIAGNOSTICS: Inspect the broadband time courses
           ...
           if savePlot
               ...
               % Plot all channels
               figure('Name', sprintf('allchannels_broadband'))
               set(gcf, 'Position', get(0, 'Screensize'));

               % Plot the voltage time courses 
               subplot(2,1,1); hold on
               plot(t,data(chan_index,:))
               xlim([min(t),max(t)])
               xlabel('Time (s)'); ylabel('Voltage');set(gca, 'FontSize', 16);
               title(sprintf('All channels, raw %s', description));

               % Plot the broadband timecourse 
               subplot(2,1,2),hold on
               plot(t,broadband(chan_index,:))
               xlim([min(t),max(t)])
               xlabel('Time (s)'); ylabel('Broadband estimate');set(gca, 'FontSize', 16);
               title(sprintf('All channels, %s \n %s', 'broadband timecourse', methodstr));
               
               % Generate a name for the figure
               figureName = fullfile(figSaveDir,sprintf('%s_allchannels',fname_out));
