...
%%
fig = figure('Color', [1 1 1], 'Position', [30 300 700 200]);
set(fig,'Units', 'Pixels', 'PaperPositionMode','Auto','PaperUnits','points','PaperSize',[700 390])

%Stimulus sequence
stimSeq         = finer_t > 0 & finer_t <= stimdur * samples;

subplot(2,6,1)
plot(finer_t, stimSeq, 'k', 'LineWidth', 3)
box off; axis off
xlim([0 2*samples])
...
subplot(2,6,3)
plot(finer_t, numResp, 'k', 'LineWidth', 3)
hold on,
plot(finer_t, stimSeq, 'k', 'LineWidth', 2)
box off; axis off
xlim([0 2*samples])
title('num resp^2', 'FontSize', 14)

% convolve with delayed response
h2 = normSum(exp(-x_data * dt / tau2));
...
subplot(2,6,5)
plot(finer_t, demResp, 'k', 'LineWidth', 3)
hold on,
plot(finer_t, stimSeq, 'k', 'LineWidth', 2)
box off; axis off
xlim([0 2*samples])
