function tt_plotData(data, channels, t, opts, savePlot, saveStr)

% Generate one plot of selected data:
% each condition as a separate timecourse, all areas/electrodes superimposed
...
if opts.average_elecs
	figureName = sprintf('selecteddata_bystimulus_electrodeaverages%s', saveStr);
    FontSz = 20;
    FigSz = [400 200 2000 1200];
else
	figureName = sprintf('selecteddata_bystimulus_individualelecs%s', saveStr);
    FontSz = 12;
...
colors = copper(length(opts.stim_names));
...
set(gca, 'xtick', (0:length(opts.stim_names))*size(data,1)+1, 'xgrid', 'on', 'xticklabel', opts.stim_names, 'xticklabelrotation', 45);
axis tight
set(gcf, 'Position', FigSz);
if opts.average_elecs
