function [results] = tt_plotParams(results, channels, options, saveDir, opts)
...
if ~isfield(opts, 'plotindivpoints'), opts.plotindivpoints = true;end
...
for kk = 1:nModels
    ...
    tmp = loadjson(fullfile(tt_RootPath, 'models', sprintf('%s.json', modelNames{kk})));
    ...
    for p = 1:nParams
        ...
        if ~dataWasAveraged
            dat = results(kk).params(p,:);
            m = mean(dat);
            se = std(dat);
            errorbar(1, m, se, '.b', 'MarkerSize', 30, 'LineWidth', 4, 'LineStyle', 'none', 'CapSize', 0)
            if opts.plotindivpoints
                scatter(ones(size(dat))+0.2, dat,40, [0.7 0.7 0.7], 'filled')
            end
        else
            m = results(kk).params(p,:);
            plot(1:nChans, m, '.k', 'MarkerSize', 30, 'LineWidth', 2, 'LineStyle', 'none')
        end
        set(gca, 'Xlim', [0 2], 'XTick', 1, 'XTickLabel', [], 'XTickLabelRotation', 45);
        if p == nParams
            set(gca, 'Ylim', [tmp.lb(p), ceil(max(dat))])
        else
            set(gca, 'Ylim', [tmp.lb(p), tmp.ub(p)])
        end
        title(paramNames{p}); xlabel('tactile area'); ylabel('parameter value');set(gca, 'fontsize', 16);
    end
    set(gcf, 'Position', [400 200 2000 1200]);
