...
writePath = fullfile(projectDir, 'derivatives', 'ECoGFigures', 'figure', subject,'prepareData');
...
% Epoch the data
% epoch: time series x trial x channel
...
% Check if the next trial starts before the end of current epoch
% If so, zero-pad the overlap for this trial and all the channels
fprintf('[%s] Zero-pad the overlap if the next trial starts before the current epoch end', mfilename);
for tt = 1:(size(epochs,2)-1)
    overlap = (events.onset(tt) + specs.epoch_t(2)) - events.onset(tt+1);
    if overlap > 0
        % Convert overlap duration to samples
